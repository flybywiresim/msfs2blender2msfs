...
class MATERIAL_PT_MSFSMaterials(Panel):
    ...
    def draw(self, context):
        ...
        if mat:
            ...
            if (
                mat.msfs_material_type != "NONE"
            ):  # Just a note to anyone wondering why this isn't msfs_material_type is not None, it's because the material type is a string enum property
                # Properties
                ...
                if mat.msfs_show_decal_blend_factors:
                    ...
                    if mat.msfs_material_type == "msfs_geo_decal":
                        box.prop(
                            mat, "msfs_decal_occlusion_blend_factor", text="Blast Sys"
                        )
                        box.prop(
                            mat, "msfs_decal_emissive_blend_factor", text="Melt Sys"
                        )
                    else:
                ...
                # Textures
                if (
                    mat.msfs_show_base_color_texture
                    or mat.msfs_show_comp_texture
                    or mat.msfs_show_normal_texture
                    or mat.msfs_show_emissive_texture
                    or mat.msfs_show_detail_color_texture
                    or mat.msfs_show_detail_comp_texture
                    or mat.msfs_show_detail_normal_texture
                    or mat.msfs_show_blend_mask_texture
                    or mat.msfs_show_wetness_ao_texture
                    or mat.msfs_show_dirt_texture
                    or mat.msfs_show_height_map_texture
                ):
                    box = layout.box()
                    ...
                    if mat.msfs_show_comp_texture:  # Composite
                        box.label(
                            text="Composite (Occlusion (R), Roughness (G), Metalness (B))"
                        )
                        box.template_ID(
                            mat, "msfs_comp_texture", new="image.new", open="image.open"
                        )
                    ...
                    if mat.msfs_show_emissive_texture:  # Emissive
                        if mat.msfs_material_type == "msfs_windshield":
                            ...
                            box.label(
                                text="Emissive Inside Window (RGB), Offset Time (A)"
                            )
                        else:
                    ...
                    if mat.msfs_show_detail_color_texture:  # Detail Color
                        if mat.msfs_material_type == "msfs_windshield":
                            box.label(
                                text="Details Scratch (R), Icing Mask (G), Fingerprints (B)"
                            )
                        elif mat.msfs_material_type == "msfs_parallax":
                    ...
                    if mat.msfs_show_detail_comp_texture:  # Detail Composite
                        if mat.msfs_material_type == "msfs_geo_decal":
                            ...
                            if mat.msfs_blend_mask_texture is None:
                                box.label(
                                    text="Detail Composite (Occlusion (R), Roughness (G), Metallic (B))"
                                )
                            else:
                                box.label(
                                    text="Secondary Occlusion (R), Roughness (G), Metallic (B)"
                                )
                        box.template_ID(
                            mat,
                            "msfs_detail_comp_texture",
                            new="image.new",
                            open="image.open",
                        )
                    ...
                    if mat.msfs_show_wetness_ao_texture:  # Wetness AO
                        if mat.msfs_material_type == "msfs_windshield":
                            ...
                            mat.msfs_material_type == "msfs_anisotropic"
                            or mat.msfs_material_type == "msfs_hair"
                        ):
                    ...
                    if mat.msfs_show_dirt_texture:  # Dirt
                        if mat.msfs_material_type == "msfs_clearcoat":
                            box.label(
                                text="Clearcoat Amount (R), Clearcoat roughness (G)"
                            )
                        box.template_ID(
                            mat, "msfs_dirt_texture", new="image.new", open="image.open"
                        )
