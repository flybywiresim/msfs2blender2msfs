...
def do_primitives(gltf, mesh_idx, skin_idx, mesh, ob):
    ...
    for prim in pymesh.primitives:
        ...
        # Asobo things
        base_vertex_index = prim.extras.get('ASOBO_primitive').get('BaseVertexIndex')
        tri_count = prim.extras.get('ASOBO_primitive').get('PrimitiveCount')
        start_index = prim.extras.get('ASOBO_primitive').get('StartIndex')
        #asobo_vertex_type = prim.extras.get('ASOBO_primitive').get('VertexType')
    ...
    for prim in pymesh.primitives:
        if prim.material is not None:
            ...
            vertex_color = None
            # MSFS Disabling vertex color import
...
def skin_into_bind_pose(gltf, skin_idx, vert_joints, vert_weights, locs, vert_normals, weight_count):
    ...
    # skinning_mats = np.zeros((num_verts, weight_count, weight_count), dtype=np.float32)
