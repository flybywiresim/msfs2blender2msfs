...
import bpy
from .gltf2_blender_custom_material_behaviors import *
...
def CreateMSFSInvisibleShader(Material):
    ...
    base_color_tint = CreateNewNode(Material,"ShaderNodeRGB","albedo_tint",location=(-300,300))
    base_color_tint.hide = True
    base_color_tint.outputs[0].default_value = (1.0,1.0,1.0,0.0)
    base_color_tint.outputs[0].default_value[3]=0.1
    links.new(base_color_tint.outputs["Color"], bsdf_node.inputs["Base Color"])
    Material.msfs_color_albedo_mix = (1.0,1.0,1.0)

    emissive_tint = CreateNewNode(Material,"ShaderNodeRGB","emissive_tint",location=(-300,-75))
    emissive_tint.hide = True
    emissive_tint.outputs[0].default_value = (0.0,0.0,0.0,1.0)
    links.new(emissive_tint.outputs["Color"], bsdf_node.inputs["Emission"])
    Material.msfs_color_emissive_mix = (0.0,0.0,0.0)
