# Copyright 2020-2021 Otmar Nitsche, FlyByWire Simulations.
#
...
from .gltf2_blender_custom_material_functions import *
...
def switch_msfs_material(self,context):
    ...
    if mat.msfs_material_type == "msfs_standard":
    ...
    elif mat.msfs_material_type == "msfs_decal":
        ...
        switch_msfs_blendmode(self, context)
...
def match_metallic(self, context):
    ...
    if metallic != None:
        ...
        if mat.msfs_metallic_texture != None:
            ...
            if mat.msfs_detail_metallic_texture != None: # if there is no detail metallic texture, we don't need to put the metallic_detail_mix in to the metallic sep node
...
def match_blend_mask(self, context):
    ...
    if nodes.get("blend_mask", None) != None:
        ...
        if mat.msfs_blend_mask_texture != None:
            if mat.msfs_blend_mask_texture.channels > 3:
                if not all(i == 1.0 for i in list(mat.msfs_blend_mask_texture.pixels)[3::4]): # checks if albedo channel is unused (texconv sets a blank albedo channel for textures that dont have one)
                    if albedo_detail_mix != None:
...
def match_behind_glass(self,context):
    ...
    if behind_glass != None:
        ...
        if mat.msfs_behind_glass_texture != None:
            # Create the link:
...
def switch_msfs_blendmode(self,context):
    mat = context.active_object.active_material
    if mat.msfs_material_type == "msfs_windshield" or mat.msfs_blend_mode == "BLEND":
        MakeTranslucent(mat)
...
Material.msfs_material_type =  bpy.props.EnumProperty(items=(("NONE","Disabled",""),
...
# Note: they don"t control the actually material parameters, only whether or 
...
Material.msfs_show_alpha_mix = bpy.props.BoolProperty(name="show_alpha_mix",default=False)
